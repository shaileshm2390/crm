...
exports.getReports = function (req, res) {
    var customQuery = 'SELECT r.id, r.subject, rp.id, rp.partName, rp.createdAt, c.data, CONCAT_WS(" ", u.firstName, u.lastName) AS assignedUser, b.name buyerName, b.email buyerEmail, (CASE WHEN (SELECT Count(id) FROM RfqFeasibilities Where RfqId = r.id) > 0 THEN "Done" WHEN (SELECT Count(id) FROM RfqFeasibilities Where RfqId = r.id) = 0 THEN "Pending" END) as  FeasibilityCheck, (SELECT Date(createdAt) FROM CostSheets Where RfqId = r.id AND RfqPartId = rp.id ORDER BY createdAt DESC limit 1 ) as LatestCostsheetPrepared, (SELECT Date(createdAt) FROM CostSheets Where RfqId = r.id AND RfqPartId = rp.id AND status="approved" ORDER BY createdAt DESC  limit 1) as LatestCostsheetApproved,  (SELECT Date(createdAt) FROM Quotations Where RfqId = r.id  ORDER BY createdAt DESC limit 1 ) as QuotationsPrepared, (CASE  WHEN (SELECT Count(id) FROM Samplesubmissionimages Where RfqId = r.id AND RfqPartId = rp.id) > 0 THEN ( CASE WHEN (SELECT Count(id) FROM Samplesubmissions Where RfqId = r.id AND RfqPartId = rp.id) THEN  (SELECT CONCAT_WS(" ", stage, stageProcess) FROM Samplesubmissions Where RfqId = r.id AND RfqPartId = rp.id LIMIT 1) ELSE "Drawing" END) ELSE "Pending" END) as  SampleStatus, (SELECT Date(createdAt) FROM PurchaseOrders Where RfqId = r.id  ORDER BY createdAt DESC limit 1) as PurchaseOrder,  (SELECT STATUS  FROM SampleInspectionReports Where RfqId = r.id AND RfqPartId = rp.id ORDER BY createdAt DESC limit 1) as SIR, (SELECT Date(createdAt)  FROM HandoverSubmitteds Where RfqId = r.id limit 1) as HandoverSubmitteds, (SELECT expectedLeadTime  FROM DeveloperHandovers Where RfqId = r.id AND RfqPartId = rp.id limit 1) as ExpectedLeadTime FROM `Rfqs` r  LEFT JOIN Users u ON u.id = r.UserId LEFT JOIN Buyers b ON b.Id = r.BuyerId LEFT JOIN RfqParts rp ON rp.RfqId = r.id LEFT JOIN CostSheets c ON c.RfqId = r.id AND c.RfqPartId = rp.id WHERE DATE(r.createdAt) BETWEEN "' + req.query.fromDate + '" AND "' + req.query.toDate + '" ';
